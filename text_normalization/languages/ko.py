"""Korean text normalization for standalone integer tokens."""
...
class KoreanCardinalConfig:
    """Configuration surface for Korean cardinal normalization."""

    max_scale_groups: int = len(LARGE_UNITS)


DEFAULT_CONFIG = KoreanCardinalConfig()


def normalize_integer_token(
    token: str, config: KoreanCardinalConfig = DEFAULT_CONFIG
) -> str:
...
def verbalize_integer(
    value: int, config: KoreanCardinalConfig = DEFAULT_CONFIG
) -> str:
    if value == 0:
        return DIGITS[0]
    ...
    while remaining > 0:
        ...
        if scale_index >= config.max_scale_groups:
            raise ValueError(
                f"Value {value} exceeds configured Korean scale groups "
